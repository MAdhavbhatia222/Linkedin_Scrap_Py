...
All_Data_All_Job_Page = pd.read_csv(r"Path_to_csv.csv")

# Extract the 'Link' column from the loaded DataFrame
All_Links = All_Data_All_Job_Page['Link']
...
for profile_url in All_Links[34:]:
    ...
    try:
        ...
        Job_Name = soup.findAll('div', {'class': 'display-flex justify-space-between align-items-center'})[0].text.strip()
        Job_Primary_Details = soup.findAll('div', {'class': 'jobs-unified-top-card__primary-description'})[0].text.strip()
        Job_Contract = soup.findAll('li', {'class': 'jobs-unified-top-card__job-insight'})[0].text.strip()
        ...
        if len(Skills_all) > 0:
            skills_meet = Skills_all[0].text.strip().replace(", and", ",")
        ...
        All_Job_Details.to_csv("All_Job_Details_" + today_date + ".csv", index=False)
    except:
        # Handle failed links
